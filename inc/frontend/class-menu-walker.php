...
if ( ! class_exists( 'Inkpro_Custom_Fields_Nav_Walker' ) ) {
	/**
	 * @since InkPro 1.0.0
	 */
	class Inkpro_Custom_Fields_Nav_Walker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( get_post_meta( $item->ID, '_mega-menu-tagline', true ) ) {
			...
			if ( get_post_meta( $item->ID, '_menu-item-new', true ) ) {
				$classes[] = 'new';
			}

			if ( get_post_meta( $item->ID, '_menu-item-hot', true ) ) {
				$classes[] = 'hot';
			}
			...
			$mega_menu_bg = ( $img_id ) ? esc_url( wolf_get_url_from_attachment_id( absint( $img_id ), 'inkpro-XL' ) ) : '';
			...
			$mega_menu_tagline = get_post_meta( $item->ID, '_mega-menu-tagline', true );
			$data_mega_menu_tagline = ( $mega_menu_tagline ) ? ' data-mega-menu-tagline="' . esc_attr( $mega_menu_tagline ) . '"' : '';

			$classes[] = 'mega-menu-bg-' . $bg_repeat; // bg repeat
			...
			// mega menu columns
			$mega_menu_cols = ( get_post_meta( $item->ID, '_mega-menu-cols', true ) ) ? get_post_meta( $item->ID, '_mega-menu-cols', true ) : 4;
			...
			$output .= $indent . '<li' . $id . $class_names . $data_bg . $data_bg_repeat . $data_mega_menu_tagline . '>';
			...
			if ( get_post_meta( $item->ID, '_menu-item-scroll', true ) ) {
				$item_output .= '<a'. $attributes .' class="menu-link scroll"><span class="menu-item-inner"><span class="menu-item-text-container" itemprop="name">';
			} else {
				$item_output .= '<a'. $attributes .' class="menu-link"><span class="menu-item-inner"><span class="menu-item-text-container" itemprop="name">';
			}
